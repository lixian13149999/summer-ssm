...
-- -----------------------------------------------------
-- Table `summer`.`wech_location`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `summer`.`wech_location` ;

SHOW WARNINGS;
CREATE TABLE IF NOT EXISTS `summer`.`wech_location` (
  `id` VARCHAR(45) NOT NULL COMMENT '地理位置信息表',
  `del_flag` INT NULL COMMENT '伪删除字段',
  `open_id` VARCHAR(145) NULL COMMENT '微信关注者的openid',
  `latitude` DOUBLE NULL COMMENT '纬度',
  `longitude` DOUBLE NULL COMMENT '经度',
  `precision` DOUBLE NULL COMMENT '地理位置的精度',
  `label` VARCHAR(145) NULL COMMENT '地理位置信息',
  `create_time` VARCHAR(45) NULL COMMENT '添加时间',
  `create_by` VARCHAR(145) NULL COMMENT '添加者',
  `update_time` VARCHAR(45) NULL COMMENT '修改时间',
